...
def realtime_growth(date_string=None, weekly=False, monthly=False):
    ...
    if weekly is True:
        ...
        intervals = pd.date_range(end=yesterday, periods=8, freq='7D').strftime(
            STRFTIME_DATA_FRAME_FORMAT).tolist()
        ...
        intervals = pd.date_range(end=yesterday, periods=3, freq='1M').strftime(
            STRFTIME_DATA_FRAME_FORMAT).tolist()
        for day in intervals:
            monthly_df = monthly_df.append(
                growth_df.loc[growth_df.index == day])
        return monthly_df
...
def percentage_trends():
    ...
    rate_change = round(((current.Deaths/current.Confirmed)*100) -
                        ((last_week.Deaths / last_week.Confirmed)*100), ndigits=2)
...
def global_cases():
    ...
    Returns:
        [pd.DataFrame]
    """
    df = daily_report()[['Country_Region', 'Confirmed',
                         'Deaths', 'Recovered', 'Active']]
    df.rename(columns={'Country_Region': 'Country'}, inplace=True)
    # Dataframe mapper, combines rows where country value is the same
...
def usa_counties():
    ...
    populations = pd.read_csv(
        'https://raw.githubusercontent.com/balsama/us_counties_data/master/data/counties.csv')[['FIPS Code', 'Population']]
    ...
    df['cases/capita'] = (df.cases / df.Population) * \
        100000  # per 100k residents
